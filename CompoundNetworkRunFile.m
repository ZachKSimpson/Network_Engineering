...
results = zeros(numIterations, 10); % Initialize results to hold both simulated and calculated values

%% Loop 
for i = 1:numIterations
    for x = 1:length(K)
        results(i, x) = runCompoundNetworkSim(K(x), (i / 100), 1000); % Simulated results
        results(i, x + 5) = K(x) / ((1 - (i / 100))^2); % Theoretical results
    end
end

%% Figure for all K
figure('Name', 'Compound Network', 'NumberTitle', 'off');
